...
class ShopkeeperDashboard {
    ...
    async loadAllCVs() {
        ...
        try {
            if (window.supabaseDatabaseManager) {
                ...
                const result = await window.supabaseDatabaseManager.searchCVs('', '', null, 'shopkeeper', this.currentUser.id, 1000, 0, false);
    ...
    async openCV(cvId) {
        ...
        try {
            ...
            const fullCVData = await window.supabaseDatabaseManager.getCVById(cvId, 'shopkeeper');
