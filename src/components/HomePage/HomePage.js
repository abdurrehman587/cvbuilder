...
const HomePage = ({ onProductSelect }) => {
  ...
  // Open shopkeeper registration form when coming from shopkeeper panel "Register as Shopkeeper" link
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const hash = (location.hash || '').toLowerCase();
    if (params.get('openShopkeeperForm') === '1' || hash === '#become-shopkeeper') {
