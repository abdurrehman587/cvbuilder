...
import usePreviewHandler from './PreviewHandler6';
// eslint-disable-next-line no-unused-vars
import generatePDF from './pdf6';
...
function Preview6({ formData: propFormData, autoSaveStatus, hasUnsavedChanges, selectedTemplate, onTemplateSwitch, isPreviewPage, updateFormData }) {
  ...
  if (!formData || (!formData.name && !formData.education?.length && !formData.experience?.length)) {
    ...
    if (storedData) {
      try {
        ...
        console.error('Preview6 - Error parsing stored data:', e);
  ...
  useEffect(() => {
    ...
    const updatePageBreaks = () => {
      ...
      const sections = previewElement.querySelectorAll('.t6-cv-section');
  ...
  const renderCVContent = () => {
    ...
    <div className="t6-root">
      <header className="t6-header">
        <div className="t6-header-row">
          {displayData.showProfileImage !== false && (
            <div className="t6-profile-photo">
              {profileImageUrl ? (
                <img src={profileImageUrl} alt="" className="t6-profile-img" />
              ) : (
                <div className="t6-profile-placeholder" aria-hidden="true" />
              )}
            </div>
          )}
          <div className="t6-header-text">
            <h1 className="t6-name">{displayData.name}</h1>
            {displayData.position ? (
              <p className="t6-tagline">{displayData.position}</p>
            ) : null}
          </div>
        </div>
        <div className="t6-header-rule" />
      </header>

      <div className="t6-split-wrapper t6-cv-section">
        <div className="t6-body">
          <div className="t6-main-column">
          <main className="t6-main">
            <section className="t6-main-block">
              ...
              <p className="t6-prose">{displayData.professionalSummary}</p>
            </section>
              ...
              <section className="t6-main-block">
                ...
                <ul className="t6-skill-list">
                  {displayData.skills.map((skill, index) => (
                    <li key={index} className="t6-skill-item">{skill}</li>
                  ))}
                </ul>
              </section>
            ) : null}
            ...
            {displayData.experience && displayData.experience.length > 0 ? (
              <section className="t6-main-block">
                ...
                <div className="t6-exp-list">
                  {displayData.experience.map((exp, index) => (
                    <article key={index} className="t6-exp-item">
                      <div className="t6-exp-title-line">
                        <span className="t6-exp-title">
                          {exp.jobTitle || 'Role'}
                          {exp.company ? (
                            <span className="t6-exp-at">{' '}at {exp.company}</span>
                          ) : null}
                        </span>
                      </div>
                      {exp.duration ? (
                        <p className="t6-exp-dates">{exp.duration}</p>
                      ) : null}
                      {exp.jobDetails ? (
                        <div className="t6-exp-desc">
      ...
      {displayData.education && displayData.education.length > 0 ? (
        <section className="t6-full t6-cv-section">
          <div className="t6-fw-heading">
            <span className="t6-icon t6-icon-square" aria-hidden="true" />
            <h2 className="t6-fw-title">Education</h2>
            <span className="t6-fw-line" aria-hidden="true" />
          </div>
          <div className="t6-edu-list">
            {displayData.education.map((edu, index) => (
              <article key={index} className="t6-edu-item">
                <h3 className="t6-edu-title">
                  {[edu.degree, edu.board].filter(Boolean).join(' — ')}
                </h3>
                {(edu.year || edu.marks) ? (
                  <p className="t6-edu-dates">
                    {[edu.year, edu.marks].filter(Boolean).join(' · ')}
                  </p>
                ) : null}
                {edu.details ? (
                  <div className="t6-prose t6-edu-desc">
                    {edu.details.split('\n').map((line, i) =>
                      line.trim() ? <p key={i}>{line.trim()}</p> : null
                    )}
                  </div>
      ...
      {displayData.certifications && displayData.certifications.length > 0 ? (
        <section className="t6-full t6-cv-section">
          <div className="t6-fw-heading">
            <span className="t6-icon t6-icon-square" aria-hidden="true" />
            <h2 className="t6-fw-title">Certifications</h2>
            <span className="t6-fw-line" aria-hidden="true" />
          </div>
          <ul className="t6-cert-list">
            {displayData.certifications.map((cert, index) => (
              <li key={index} className="t6-cert-item">{cert}</li>
            ))}
          </ul>
        </section>
      ) : null}
      ...
      {displayData.hobbies && displayData.hobbies.length > 0 ? (
        <section className="t6-full t6-cv-section">
          <div className="t6-fw-heading">
            <span className="t6-icon t6-icon-square" aria-hidden="true" />
            <h2 className="t6-fw-title">Hobbies</h2>
            <span className="t6-fw-line" aria-hidden="true" />
          </div>
          <p className="t6-inline-list">{displayData.hobbies.join(' · ')}</p>
        </section>
      ) : null}
      ...
      {displayData.references && displayData.references.length > 0 ? (
        <section className="t6-full t6-cv-section">
          <div className="t6-fw-heading">
            <span className="t6-icon t6-icon-square" aria-hidden="true" />
            <h2 className="t6-fw-title">References</h2>
            <span className="t6-fw-line" aria-hidden="true" />
          </div>
          <div className="t6-prose">
          ...
          <aside className="t6-sidebar">
            <h3 className="t6-sidebar-title">Contact Information</h3>
            {displayData.phone ? (
              <div className="t6-detail-block">
                <span className="t6-detail-label">Phone</span>
                <span className="t6-detail-value">{displayData.phone}</span>
              </div>
            ) : null}
            {displayData.email ? (
              <div className="t6-detail-block">
                <span className="t6-detail-label">Email</span>
                <a className="t6-link" href={`mailto:${displayData.email}`}>{displayData.email}</a>
              </div>
            ) : null}
            {displayData.address ? (
              <div className="t6-detail-block">
                <span className="t6-detail-label">Address</span>
                <span className="t6-detail-value">{displayData.address}</span>
              </div>
            ) : null}

            {displayData.languages && displayData.languages.length > 0 ? (
              <>
                <h3 className="t6-sidebar-title t6-sidebar-title--spaced">Languages</h3>
                <ul className="t6-lang-list">
                  {displayData.languages.map((language, index) => {
                    ...
                    return (
                      <li key={index} className="t6-lang-item">
                        <span className="t6-lang-name">{languageName}</span>
                        {languageLevel ? <span className="t6-lang-level">{languageLevel}</span> : null}
                      </li>
                    );
                  })}
                </ul>
            ...
            {(() => {
              ...
              return (
                <>
                  <h3 className="t6-sidebar-title t6-sidebar-title--spaced">Links</h3>
                  <ul className="t6-links-list">
                    {links.map((info, index) => (
                      <li key={index}>
                        <a className="t6-link" href={info.value.trim()} target="_blank" rel="noopener noreferrer">
            ...
            {(() => {
              ...
              return (
                <>
                  <h3 className="t6-sidebar-title t6-sidebar-title--spaced">Other Information</h3>
                  {plain.map((info, index) => (
                    <div key={index} className="t6-detail-block">
                      <span className="t6-detail-label">{info.label}</span>
                      <span className="t6-detail-value">{info.value}</span>
                    </div>
                  ))}
                </>
              );
  ...
  if (isPreviewPage) {
    return (
      <div className="preview-page-preview-wrapper">
        <div 
          className="t6-preview t6-a4-size-preview t6-pdf-mode preview-page-preview"
          style={{
  ...
  return (
    <>
      ...
      <div 
        className="t6-preview t6-a4-size-preview t6-pdf-mode"
        style={{
