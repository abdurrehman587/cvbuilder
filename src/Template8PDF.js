...
const Template8PDF = ({ formData, visibleSections = [] }) => {
  ...
  useEffect(() => {
    const updateButtonText = async () => {
      try {
        ...
        const text = await PaymentService.getDownloadButtonText('template8', isAdmin);
        setButtonText(text);
      } catch (error) {
  ...
  const handleDownloadClick = async () => {
    try {
      ...
      if (approvedPayment) {
        ...
        await PaymentService.markPaymentAsUsed(approvedPayment.id, 'template8');
        generatePDF();
