...
const Template9PDF = ({ formData, visibleSections = [] }) => {
  ...
  const renderCustomSections = (customSections) => {
    console.log('Template9PDF - renderCustomSections called with:', customSections);
    
    if (!customSections || customSections.length === 0) {
      console.log('Template9PDF - renderCustomSections: no customSections or empty array');
      return null;
    }

    return customSections.map((section, sectionIndex) => {
      console.log(`Template9PDF - processing section ${sectionIndex}:`, section);
      ...
      if (!hasTitle || validDetails.length === 0) {
        console.log(`Template9PDF - section ${sectionIndex} hidden: no title or no valid details`);
        return null;
      }
      
      console.log(`Template9PDF - rendering section ${sectionIndex}:`, sectionTitle);
      return (
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      await html2pdf()
        .set({
          margin: 10,
          filename: `${formData.name || 'CV'}_template9.pdf`,
  ...
  return (
    <article ref={containerRef} style={containerStyle}>
      ...
      {(() => {
        console.log('Template9PDF - customSections check:', {
          visibleSections,
        ...
        console.log('Template9PDF - shouldShowCustomSections:', shouldShowCustomSections);
